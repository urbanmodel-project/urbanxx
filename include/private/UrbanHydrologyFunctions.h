...
// Compute derivative of hydraulic conductivity with respect to volumetric water content
// dhk/dw = imped * (2b+3) * hksat * s^(2b+2) / watsat
...
// smp = -sucsat * s^(-b)
// where s = relative saturation (vol_liq / watsat)
KOKKOS_INLINE_FUNCTION Real ComputeMatricPotential(
    const Real s,           // relative saturation [-]
...
// Compute derivative of matric potential with respect to volumetric water content
// dsmp/dw = -b * smp / (s * watsat)
...
KOKKOS_INLINE_FUNCTION Real ComputeEquilibriumWaterContent(
  ...
  if (zwt <= z_top) {
    ...
    const Real temp_0 = Kokkos::pow((sucsat + zwt - z_top) / sucsat, 
                                    1.0 - 1.0 / bsw);
    const Real vol_eq1 = -sucsat * watsat / (1.0 - 1.0 / bsw) / 
                         (zwt - z_top) * (temp_i - temp_0);
    
    // Weighted average of saturated and unsaturated parts
    vol_eq = (vol_eq1 * (zwt - z_top) + watsat * (z_bot - zwt)) / 
             (z_bot - z_top);
    vol_eq = Kokkos::fmin(watsat, Kokkos::fmax(vol_eq, 0.0));
    
  } else {
    // Layer is fully unsaturated
    const Real temp_i = Kokkos::pow((sucsat + zwt - z_bot) / sucsat, 
                                    1.0 - 1.0 / bsw);
    const Real temp_0 = Kokkos::pow((sucsat + zwt - z_top) / sucsat, 
                                    1.0 - 1.0 / bsw);
