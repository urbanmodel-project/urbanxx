...
void UrbanInitializeTemperature(UrbanType urban, UrbanErrorCode *status) {
  ...
  try {
    ...
    fflush(stdout);
    Kokkos::parallel_for(
        "InitRoofTemp", Kokkos::RangePolicy<ExecSpace>(0, numLandunits),
        KOKKOS_LAMBDA(int l) { roofTempPtr[l] = TEMP_ROOF_INIT; });
    Kokkos::fence();
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(int l) { imperviousRoadTempPtr[l] = TEMP_ROAD_INIT; });
    Kokkos::fence();
    ...
    Kokkos::parallel_for(
        ...
        Kokkos::RangePolicy<ExecSpace>(0, numLandunits), KOKKOS_LAMBDA(int l) {
          ...
          tafPtr[l] =
              TEMP_CANYON_AIR_INIT; // Initialize to canyon air temperature
          qafPtr[l] =
              QAF_INIT; // Initialize to reasonable humidity value (kg/kg)
        });
